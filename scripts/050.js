...
export default function sc050(engine) {
    ...
    BABYLON.SceneLoader.LoadAssetContainer("../assets/", "050-post_election.glb", scene, function(container) {
        ...
        camAnim(0.5, 1.25, 3.5, "TGT.Start", 1.25);
        ...
        votes.actionManager.registerAction(new BABYLON.ExecuteCodeAction(BABYLON.ActionManager.OnPickTrigger, function (pvda) {
                camAnim(1.9, 1.25, 5, "TGT.Votes", 0.75);
                dialogPop("Stemmen Tellen", "Het tellen van de stemmen gebeurt dankzij moderne technologie steeds sneller. Doorheen de dag kan je volgen hoe mondjesmaat de uitslagen van verschillende gemeenten bekendgemaakt worden. 's Avonds zijn dan alle stemmen geteld en we weten al meteen hoeveel zetels de partijen gewonnen hebben en wie de meeste voorkeurstemmen gehaald heeft. Maar dat wil zeggen dat het harde werk nog maar net begonnen is.", false);
                main.setY(votes);
